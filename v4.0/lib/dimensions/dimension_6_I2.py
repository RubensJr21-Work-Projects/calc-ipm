...
def I2(base_main: pd.DataFrame, base_familias: pd.DataFrame) -> pd.DataFrame:
    ...
    b_familias["bool_Banheiro_COMP"] = b_familias["21 - Existe banheiro de uso exclusivo, com chuveiro e vaso sanitário, neste domicílio, inclusive os localizados no terreno?*"].fillna('').eq('Não')
    
    values_for_opt_Escoamento = [
        "Fossa rudimentar",
        "Vala a céu aberto",
