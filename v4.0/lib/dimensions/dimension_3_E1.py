...
def E1(base_main: pd.DataFrame, base_familias: pd.DataFrame, base_membros: pd.DataFrame) -> pd.DataFrame:
    ...
    # 1.
    # 1.1 Verificar se M19 != ["Ensino Médio EJA (Supletivo)", "Superior, Aperfeiçoamento, Especialização, Mestrado, Doutorado"] OR M19.1 != "3º ano (curso completo)"
    ...
    b_membros["Não concluiu o ensino médio"] = (
        ...
        |
        ~b_membros["M19.1 - E qual foi a última série que foi aprovado?"].fillna("").eq("3º ano (curso completo)")
    )
    
    # 1.2 Verificar se 4 <= Idade <= 17
    b_membros["4 <= Idade <= 17"] = b_membros["Idade"].between(4, 17, inclusive="both")

    # 1.3 Verificar se M16 == "Não" OR M17 == "Sim" OR M18 == "Não"
    b_membros["Fora da escola ou em risco"] = (
        b_membros["M16 - Frequenta regularmente a escola? (exclusiva para MENOR de 18 anos)"].fillna("").eq("Não")
        |
        b_membros["M17 - Está em risco de exclusão escolar? (exclusiva para MENOR de 18 anos)"].fillna("").eq("Sim") # Deveria ser S
        |
        b_membros["M18 - Está matriculado na escola? (exclusiva para MENOR de 18 anos)"].fillna("").eq("Não")
    )
    ...
    # 2.3 M15 - Está estudando? == "Não"
    b_membros["Não está estudando"] = b_membros["M15 - Está estudando?"].fillna("").eq("Não")
